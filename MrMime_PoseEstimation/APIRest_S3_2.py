...
def write_vector_data(arr, archi):
    ...
    linea = str(v1[0]) +" "+ str(v1[1])+" "+str(v1[2])+" "+str(v2[0]) +" "+ str(v2[1])+" "+str(v2[2])
    #mano izquierda
...
def uploadResult(video_name):
  ...
  archi.write("se1_x se1_y se1_z eh1_x eh1_y eh1_z se2_x se2_y se2_z eh2_x eh2_y eh2_z")
  archi.write('\n')
  ...
  model1.load_weights("./neural_networks/model.h5")
  
  #print(model1.predict_classes([[[0,0,-1]]])[0])
  json_file = open('./neural_networks/model2.json', 'r')
  loaded_model_json = json_file.read()
  json_file.close()
  model2 = model_from_json(loaded_model_json)
  model2.load_weights("./neural_networks/model2.h5")
  ...
  while cap.isOpened():
    ...
    print(i)
    write_vector_data(pose_3d, archi)
  archi.close()
  ...
  for i in df.index:
    #right
    rse = [df["se1_x"][i],df["se1_y"][i],df["se1_z"][i]]
    right_se = model1.predict_classes([[rse]])
    reh = [df["eh1_x"][i],df["eh1_y"][i],df["eh1_z"][i]]
    ...
    #left
    lse = [df["se2_x"][i]*-1,df["se2_y"][i],df["se2_z"][i]]
    left_se = model1.predict_classes([[lse]])
    leh = [df["eh2_x"][i]*-1,df["eh2_y"][i],df["eh2_z"][i]]
